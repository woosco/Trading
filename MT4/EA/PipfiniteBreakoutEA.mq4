...
void Indicators()
{
	...
	// SCW changed logic so that energy level 4/-4 is ignored as well as that's overbought oversold from the energy meter    
	//Buy  = (buf11 != EMPTY_VALUE && SM1 >= ThresholdLevel);  
	//Sell = (buf22 != EMPTY_VALUE && SM2 <= -ThresholdLevel); 
	Buy = ((buf11 != EMPTY_VALUE) && ((SM1 >= ThresholdLevel) && (SM1 < 4)));
	Sell = ((buf22 != EMPTY_VALUE) && ((SM2 <= -ThresholdLevel) && (SM2 > -4)));
